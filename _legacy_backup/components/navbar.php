...
// Verifica se o usuário logado é admin e conta pendentes
$isAdmin = false;
$totalPendentes = 0;
...
<nav class="w-full bg-white border-b border-slate-200 shadow-sm flex items-center justify-between px-4 md:px-6 py-2 sticky top-0 z-50">
    <div class="flex items-center gap-3">
        <div class="relative">
            ...
            <i class="fa-solid fa-hat-wizard text-red-500 absolute -top-2 -right-1 text-sm transform rotate-12"></i>
        </div>
        <a href="<?= $path_prefix ?>dashboard.php" class="text-xl md:text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <i class="fa-solid fa-tree text-green-500 text-lg animate-pulse"></i>
            CereniaPet
            <i class="fa-solid fa-gift text-red-500 text-lg"></i>
        </a>
    </div>
    ...
    <div id="navbar-links" class="hidden md:flex flex-col md:flex-row md:items-center gap-2 absolute md:static top-16 left-0 w-full md:w-auto bg-white md:bg-transparent shadow-lg md:shadow-none z-50 md:z-auto p-4 md:p-0">
        ...
        <?php if ($isAdmin): ?>
        <a href="<?= $path_prefix ?>dashboard/gerenciar_usuarios.php" data-page="gerenciar_usuarios.php" class="nav-link text-slate-600 hover:bg-slate-100 hover:text-blue-600 font-semibold transition flex items-center gap-2 px-3 py-2 rounded-md relative">
            <i class="fa-solid fa-user-shield w-4 text-center"></i><span>Usuários</span>
            <?php if ($totalPendentes > 0): ?>
            <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center"><?= $totalPendentes ?></span>
            <?php endif; ?>
        </a>
        <?php endif; ?>
