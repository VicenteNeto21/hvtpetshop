...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $telefone_nao_informado = isset($_POST['telefone_nao_informado']);
    $telefone_tutor = $telefone_nao_informado ? null : $_POST['telefone_tutor'];
...
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-200 flex flex-col">
        ...
        <div class="bg-white p-6 md:p-8 rounded-lg shadow-sm animate-fade-in">
            <form action="" method="POST" onsubmit="return validarFormulario()">
                <div class="grid grid-cols-1 lg:grid-cols-2 lg:gap-12">
                    ...
                    <div class="space-y-5">
                        <h3
                            class="text-lg font-semibold text-slate-700 border-b border-slate-200 pb-2 flex items-center gap-3">
                            <i class="fa fa-user text-amber-500"></i> Informações do Tutor
                        </h3>
                        <div class="space-y-4">
                            <div>
                                <label for="nome_tutor" class="block text-sm font-medium text-slate-600 mb-1">Nome do
                                    Tutor *</label>
                            ...
                            <div>
                                <label for="email_tutor" class="block text-sm font-medium text-slate-600 mb-1">E-mail do
                                    Tutor (Opcional)</label>
                                <input type="email" name="email_tutor" id="email_tutor"
                                    class="w-full p-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700"
                                    placeholder="email@exemplo.com">
                            </div>
                            <div>
                                <label for="telefone_tutor"
                                    class="block text-sm font-medium text-slate-600 mb-1">Telefone do Tutor</label>
                                <input type="tel" name="telefone_tutor" id="telefone_tutor"
                                    class="w-full p-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700"
                                    placeholder="(00) 00000-0000" maxlength="15" oninput="mascaraTelefone(this)">
                            </div>
                            <div class="mt-2 flex items-center">
                                <input type="checkbox" id="telefone_nao_informado" name="telefone_nao_informado"
                                    class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                    onchange="toggleTelefone()">
                                <label for="telefone_nao_informado" class="ml-2 text-sm text-slate-600">Telefone não
                                    informado</label>
                            </div>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" id="telefone_is_whatsapp" name="telefone_is_whatsapp" value="Sim"
                                class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            <label for="telefone_is_whatsapp" class="ml-2 text-sm text-slate-600">Este número é
                                WhatsApp</label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-5 mt-8 lg:mt-0">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label for="nome" class="block text-sm font-medium text-slate-600 mb-1">Nome do Pet
                                    *</label>
                            ...
                            <div>
                                <label for="especie" class="block text-sm font-medium text-slate-600 mb-1">Espécie
                                    *</label>
                            ...
                            <div>
                                <label for="nascimento" class="block text-sm font-medium text-slate-600 mb-1">Data de
                                    Nascimento</label>
                            ...
                            <div>
                                <label for="peso" class="block text-sm font-medium text-slate-600 mb-1">Peso
                                    (kg)</label>
                            ...
                            <div class="sm:col-span-2">
                                <label for="pelagem"
                                    class="block text-sm font-medium text-slate-600 mb-1">Pelagem</label>
                            ...
                            <div class="sm:col-span-2">
                                <label for="observacoes"
                                    class="block text-sm font-medium text-slate-600 mb-1">Observações</label>
    ...
    <script>
        ...
        function toggleTelefone() {
            const checkbox = document.getElementById('telefone_nao_informado');
            const telefoneInput = document.getElementById('telefone_tutor');
            const whatsappCheckbox = document.getElementById('telefone_is_whatsapp');

            if (checkbox.checked) {
                telefoneInput.value = '';
                telefoneInput.disabled = true;
                telefoneInput.required = false;
                telefoneInput.classList.add('bg-slate-100', 'cursor-not-allowed');
                whatsappCheckbox.disabled = true;
                whatsappCheckbox.checked = false;
            } else {
                telefoneInput.disabled = false;
                telefoneInput.required = false;
                telefoneInput.classList.remove('bg-slate-100', 'cursor-not-allowed');
